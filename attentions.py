...
class Encoder(nn.Module):
    ...
    def setup(self):
        ...
        for i in range(self.n_layers):
            attn_layers.append(MultiHeadAttention(
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None, deterministic: bool = True):
        ...
        scores = jnp.matmul(key.transpose(0, 1, 3, 2), query / math.sqrt(self.k_channels))
        if self.window_size is not None:
        ...
        output = jnp.matmul(value, p_attn)
        if self.window_size is not None:
            relative_weights = self._absolute_position_to_relative_position(p_attn)
            value_relative_embeddings = self._get_relative_embeddings(self.emb_rel_v, t_s)
            output += self._matmul_with_relative_values(relative_weights, value_relative_embeddings)

        output = output.reshape(b, t_t, d)
        return output
    ...
    def _matmul_with_relative_keys(self, x, y):
        ...
        ret: [b, h, l, d]
        """
        ret = jnp.matmul(x.transpose(0, 1, 3, 2), y.transpose(0, 2, 1))
        return ret
...
class FFN(nn.Module):
    ...
    def setup(self):
        self.conv1 = nn.Conv(self.filter_channels, (self.kernel_size,), dtype=self.dtype)
